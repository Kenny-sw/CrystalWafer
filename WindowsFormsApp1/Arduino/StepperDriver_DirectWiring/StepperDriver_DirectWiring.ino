...
// (без оптронов - пины Arduino идут напрямую в драйвер)
// 
// Моя плата: Arduino Nano/Uno + драйверы шаговых (TB6600/DM542/A4988)
// Логика: активный уровень HIGH (стандартно для большинства драйверов)
// ========================================================================

// -------------------- Назначение пинов --------------------
const uint8_t PIN_STEP_X = 2;  // Импульсы шагов для оси X
const uint8_t PIN_STEP_Y = 3;  // Импульсы шагов для оси Y

const uint8_t PIN_DIR_X  = 4;  // Направление движения оси X
const uint8_t PIN_DIR_Y  = 5;  // Направление движения оси Y

const uint8_t PIN_ENA    = 6;  // Включение/выключение моторов (фиксация стола)

const uint8_t PIN_EDGE   = 7;  // Датчик края пластины (оптический/индуктивный)

//  true = пластина обнаружена
const bool EDGE_ACTIVE_HIGH = true;

// -------------------- Настройки профиля движения --------------------
// Драйверу нужен импульс минимум 2-5 мкс и паузы между шагами
const uint16_t STEP_PULSE_US = 5;   // Длительность импульса STEP (надёжно работает)
...
void setLock(bool locked) {
  // HIGH на PIN_ENA = моторы включены, стол зафиксирован
  // LOW на PIN_ENA = моторы выключены, стол свободен
...
public:
  ...
  Axis(uint8_t d, uint8_t s) : dirPin(d), stepPin(s) {
    ...
    digitalWrite(dirPin, LOW);   // Начальное направление
    digitalWrite(stepPin, LOW);  // STEP в покое LOW
  }

  // Установить направление движения
  // dirHigh=true -> едем в одну сторону, false -> в другую
  ...
  inline void pulseStep() {
    digitalWrite(stepPin, HIGH);  // Импульс вверх
    delayMicroseconds(STEP_PULSE_US);
    digitalWrite(stepPin, LOW);   // Импульс вниз
  }
...
void setup() {
  ...
  digitalWrite(PIN_ENA, LOW);  // Старт: стол свободен (моторы выключены)
  g_isLocked = false;
